...
export class AlertDialogComponent {
  ...
  close(): void {
    const el = this.dialogEl().nativeElement;
    if (typeof (el as any).close === 'function') { (el as any).close(); } else { el.removeAttribute('open'); }
  }
  confirm(): void { this.confirmed.emit(); this.close(); }
}

@Component({ selector: 'fb-alert-dialog-header', host: {class: 'flex flex-col gap-1.5 mb-4 block'}, template: '<ng-content />' })
export class AlertDialogHeaderComponent {}

@Component({ selector: 'fb-alert-dialog-title', host: {class: 'text-lg font-semibold leading-none tracking-tight block'}, template: '<ng-content />' })
export class AlertDialogTitleComponent {}

@Component({ selector: 'fb-alert-dialog-description', host: {class: 'text-sm text-muted-foreground block'}, template: '<ng-content />' })
export class AlertDialogDescriptionComponent {}

@Component({ selector: 'fb-alert-dialog-footer', host: {class: 'flex justify-end gap-2 mt-6 block'}, template: '<ng-content />' })
...
export class AlertDialogCancelComponent { dialog = inject(ALERT_DIALOG_CONTEXT); }

@Component({
  selector: 'fb-alert-dialog-action',
  template: `<button class="btn btn-destructive" (click)="dialog.confirm()"><ng-content /></button>`,
})
export class AlertDialogActionComponent { dialog = inject(ALERT_DIALOG_CONTEXT); }
