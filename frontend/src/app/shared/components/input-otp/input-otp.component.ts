  ...
  template: `
    <div class="flex items-center gap-2">
      @for (i of slots(); track i) {
        <input #otpInput type="text" inputmode="numeric" maxlength="1"
          ...
          [value]="values()[i]" [disabled]="isDisabled()"
          (input)="onInput(i, $event)" (keydown)="onKeydown(i, $event)" (paste)="onPaste($event)" />
      }
    </div>
  `,
  providers: [{provide: NG_VALUE_ACCESSOR, useExisting: forwardRef(() => InputOtpComponent), multi: true}],
})
export class InputOtpComponent implements ControlValueAccessor {
  ...
  onInput(index: number, event: Event): void {
    ...
    this.values.update(v => { const next = [...v]; next[index] = char; return next; });
    this.emitValue();
    if (char && index < this.length() - 1) this.focusAt(index + 1);
  }

  onKeydown(index: number, event: KeyboardEvent): void {
    if (event.key === 'Backspace' && !this.values()[index] && index > 0) {
      this.values.update(v => { const next = [...v]; next[index - 1] = ''; return next; });
      this.emitValue();
  ...
  onPaste(event: ClipboardEvent): void {
    ...
    const chars = (event.clipboardData?.getData('text') ?? '').split('').slice(0, this.length());
    this.values.set(Array.from({length: this.length()}, (_, i) => chars[i] ?? this.values()[i] ?? ''));
    this.emitValue();
  }
