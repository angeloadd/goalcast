...
@Component({ selector: 'fb-menubar', host: {class: 'flex items-center gap-1 rounded-md border border-border bg-background p-1'}, template: '<ng-content />' })
...
@Component({
  ...
  host: {class: 'flex items-center rounded-sm px-3 py-1.5 text-sm font-medium cursor-pointer outline-none hover:bg-accent/10 transition-colors', '(click)': 'context.toggle()'},
  template: '<ng-content />',
})
export class MenubarTriggerComponent { context = inject(MENUBAR_MENU_CONTEXT); }
...
export class MenubarContentComponent { context = inject(MENUBAR_MENU_CONTEXT); }
...
export class MenubarItemComponent { context = inject(MENUBAR_MENU_CONTEXT); }

@Component({ selector: 'fb-menubar-separator', host: {class: 'block -mx-1 my-1 h-px bg-border'}, template: '' })
