  ...
  template: `
    <nav class="flex items-center justify-center gap-1">
      ...
      @for (page of pages(); track page) {
        <button class="inline-flex items-center justify-center rounded-md text-sm font-medium h-9 w-9 cursor-pointer transition-colors"
          [class.bg-accent]="page === currentPage()" [class.text-accent-foreground]="page === currentPage()"
          [class.hover:bg-muted]="page !== currentPage()" (click)="goToPage(page)">{{ page }}</button>
      }
      <button data-next
        class="inline-flex items-center justify-center rounded-md text-sm font-medium h-9 px-3 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed hover:bg-muted transition-colors"
