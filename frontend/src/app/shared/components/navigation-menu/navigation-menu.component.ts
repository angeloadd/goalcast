...
@Component({ selector: 'fb-navigation-menu', host: {class: 'relative flex items-center gap-1'}, template: '<ng-content />' })
...
@Component({
  ...
  host: {class: 'inline-flex items-center justify-center rounded-md px-4 py-2 text-sm font-medium transition-colors hover:bg-accent/10 cursor-pointer', '(click)': 'context.toggle()'},
  template: '<ng-content />',
})
export class NavigationMenuTriggerComponent { context = inject(NAV_MENU_ITEM_CONTEXT); }
...
export class NavigationMenuContentComponent { context = inject(NAV_MENU_ITEM_CONTEXT); }

@Component({
  selector: 'fb-navigation-menu-link',
  host: {class: 'block select-none rounded-md px-3 py-2 text-sm leading-none no-underline outline-none transition-colors hover:bg-accent/10 cursor-pointer'},
  template: '<ng-content />',
})
